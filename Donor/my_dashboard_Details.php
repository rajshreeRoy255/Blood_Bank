...
<head>
    ...
    // 2  DONATION
    // pending donation
    $donation_pending="SELECT * FROM `donation_tbl` WHERE status='pending' AND user_id='$session_ID'";
    $run = mysqli_query($con, $donation_pending);
    $donation_pending = mysqli_num_rows($run);
    //  echo $donation_pending;
    // pending donation
    // pending donation
    $accepted_pending="SELECT * FROM `donation_tbl` WHERE status='accepted' AND user_id='$session_ID'";
    $run2 = mysqli_query($con, $accepted_pending);
    $donation_pending_A = mysqli_num_rows($run2);
    //  echo $donation_pending_A;
    // pending donation
    // pending donation
    $donation_rejected="SELECT * FROM `donation_tbl` WHERE status='rejected' AND user_id='$session_ID'";
